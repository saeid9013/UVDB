...
def _upload_file_sync(path: Path, api_key: str, timeout: int) -> tuple[str, str]:
    if not api_key:
        raise PixeldrainError("Pixeldrain is not configured.")
    try:
        with (
            httpx.Client(timeout=httpx.Timeout(timeout)) as client,
            path.open("rb") as contents,
        ):
            response = client.put(
...
async def upload_file(path: Path, api_key: str, timeout: int) -> tuple[str, str]:
    return await asyncio.to_thread(_upload_file_sync, path, api_key, timeout)
